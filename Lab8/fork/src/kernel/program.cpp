...
int ProgramManager::executeThread(ThreadFunction function, void *parameter, const char *name, int priority)
{
    ...
    thread->stack[2] = 0;
    thread->stack[3] = 0;
    thread->stack[4] = (int)function; //switch_thread之后的返回地址
    thread->stack[5] = (int)program_exit;  //线程执行结束之后的返回地址
    thread->stack[6] = (int)parameter;  //线程的参数
...
void load_process(const char *filename)
{
    ...
    interruptStack->esp = memoryManager.allocatePages(AddressPoolType::USER, 1);//这是用户的特权级3的栈！！
...
bool ProgramManager::copyProcess(PCB *parent, PCB *child)
{
    ...
    child->stack[2] = 0;
    child->stack[3] = 0;
    child->stack[4] = (int)asm_start_process;    //asm_switch_thread的返回地址
    child->stack[5] = 0;             // asm_start_process 返回地址
    child->stack[6] = (int)childpss; // asm_start_process 参数，就是写入栈指针的值
