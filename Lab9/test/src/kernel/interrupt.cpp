...
extern "C" void c_page_interrupt_handler()
{
    ...
    if (!flag) {
        if (cr2>=KERNEL_VIRTUAL_START) {    //内核态
            if (memoryManager.KernelphysicalPageQueue.empty()) {
                asm_halt();
            }
            item u=memoryManager.KernelphysicalPageQueue.getFront();
            memoryManager.KernelphysicalPageQueue.pop();
            memoryManager.swapOut(u.virtualAddress);
            memoryManager.releasePhysicalPages(AddressPoolType::KERNEL, u.physicalAddress, 1);
            physicalPageAddress=memoryManager.allocatePhysicalPages(AddressPoolType::KERNEL, 1);
            flush_tlb_single(u.virtualAddress);
        }   
        else if (cr2>=USER_VADDR_START&&cr2<0xc0000000) {  //用户态
            if (memoryManager.UserphysicalPageQueue.empty()) {
                asm_halt();
            }
            item u=memoryManager.UserphysicalPageQueue.getFront();
            memoryManager.UserphysicalPageQueue.pop();
            memoryManager.swapOut(u.virtualAddress);
            memoryManager.releasePhysicalPages(AddressPoolType::USER, u.physicalAddress, 1);
            physicalPageAddress=memoryManager.allocatePhysicalPages(AddressPoolType::USER, 1);
            flush_tlb_single(u.virtualAddress);
    ...
    int pagenum=0;
    if (!(*pde & 0x00000001)) pagenum=0;   //无对应页表
