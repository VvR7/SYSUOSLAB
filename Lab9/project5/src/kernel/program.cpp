...
bool ProgramManager::copyProcess(PCB *parent, PCB *child)
{
    ...
    for (int i=33;i<768;i++)//从0x08400000开始，父子共享
    {
        // 无对应页表
        if (!(parentPageDir[i] & 0x1))
        {
        ...
        for (int j=0;j<1024;j++)
        {
            ...
            pageTableVaddr[j]&=0xfffffffd;  //设置为只读
            int pte = pageTableVaddr[j];
            int paddr=pte&0xfffff000;
            int index=memoryManager.read_only_page_queue.find_page(paddr);
            if (index==-1) memoryManager.read_only_page_queue.add_page(paddr);
            asm_update_cr3(childPageDirPaddr);
            memoryManager.read_only_page_queue.add_page(paddr);
            pageTableVaddr[j]=pte;   
    ...
    if (program->pageDirectoryAddress)
    {
        ...
        for (int i = 0; i < 768; ++i)
        {
            ...
            for (int j = 0; j < 1024; ++j)
            {
                ...
                if (index!=-1)
                {
                    memoryManager.read_only_page_queue.pop_page(paddr);
                }
                
                if (index==-1||memoryManager.read_only_page_queue.queue[index].count==0)
...
void ProgramManager::releaseZombie()
{
    PCB *child;
    ListItem *item;
    bool interrupt;
    ...
    while (item)
    {
        ...
        PCB *parent=get_pcb_from_pid(parentpid);
        if(parent==nullptr||parent->pid==0||parent->status==ProgramStatus::DEAD)
        {
            printf("release zombie process, pid: %d\n", child->pid);
            releasePCB(child);
...
PCB *ProgramManager::get_pcb_from_pid(int pid)
{
    ListItem *item;
    item=this->allPrograms.head.next;
    while(item)
    {
        PCB *cur=ListItem2PCB(item,tagInAllList);
        if(cur->pid==pid)
            return cur;
        item=item->next;
    }
